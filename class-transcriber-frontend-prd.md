...
- engine selectors in settings, upload, retry, diagnostics, and model management should surface runtime-available engines from the backend, including Intel GPU options such as `OpenVinoWhisperSidecar` when that runtime is installed
...
- the model manager should present the curated `OpenVinoWhisperSidecar` model set (`base-int8`, `small-fp16`, `tiny-int8`) when that engine is registered
